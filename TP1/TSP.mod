...
param M2; // número suficientemente grande como para que supere la cantidad de días que requiere todo el recorrido
param TEF; // tiempo fijo de estadía en todas las ciudades
...
var K{ i in CIUDADES}, integer; // Representa la cantidad de veces que ya se recorrieron 3000kmantes de llegar a la ciudad i.

var P, binary; // Variable bivalente utilizada solo para el equipo de Sudamérica. Si se visita Perú Antes de Febrero vale 1,
...
s.t. tiempoEstadiaAntesDeJ{j in CIUDADES_SO}: TEA[j] == sum{i in CIUDADES: i != j} (TE[i] * W[i][j]);
