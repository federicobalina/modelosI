...
class Tour(object):
	...
	#Solo se puede usar una vez, no dos seguidas porque no tengo la informacion de descanso
		...
		self.tiempoEstadia -= 7 + 5* self.descanso
		self.descansos.pop()
		if (len(self.descansos) > 0):
			self.descanso = self.descansos[len(self.descansos)-1] #CAMBIO
		else:
			self.descanso = 0
		return arista

	def esLaUltima(self,ciudad):
		for k in self.ciudadesVisitadas:
			if ( (k != ciudad) and (self.ciudadesVisitadas[k] == False) ):
				return False
		return True
	...
	def aristaEsPosible(self,arista):
		...
		if (self.AmericaSur):
			...
			if (arista.destino == ARG):
				...
				if ( (tiempoSalida >= 59) or ( (tiempoLlegada < 31) and (not(self.esLaUltima(BRA))) ) ):
					return False
